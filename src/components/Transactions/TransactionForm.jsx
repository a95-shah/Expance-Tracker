...
// Added 'transaction' prop to receive the item being edited
const TransactionForm = ({ onClose, type, transaction }) => { 
  ...
  useEffect(() => {
    if (transaction) {
      // If we have a transaction, fill the form with its data
      setFormData({
        name: transaction.name,
        description: transaction.description || '',
        amount: transaction.amount,
        category: transaction.category,
        // Convert string date (YYYY-MM-DD) back to Date object for the picker
        date: new Date(transaction.date.replace(/-/g, '/')) 
      });
    } else {
      // If adding new, reset to default
      setFormData({ 
        name: '', 
        description: '', 
        amount: '', 
  ...
  const handleSubmit = async (e) => { 
    ...
    // Format date specifically to avoid timezone issues
    ...
    try {
      if (transaction) {
        ...
        // We use .unwrap() to handle Redux Toolkit errors properly
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {type === 'expense' && (
          <div>
            ...
            <Select
                options={categoryOptions}
                // Logic to match the string value to the object react-select needs
                value={categoryOptions.find(option => option.value === formData.category)}
                onChange={(selectedOption) => setFormData({...formData, category: selectedOption.value})}
                placeholder="Select a category..."
                menuPosition="fixed"
