...
const Dashboard = () => {
  ...
  // --- CHANGED HERE: Cleaned up the edit click handler ---
  const handleEditClick = (transaction) => {
    setEditingTransaction(transaction);                 
    setFormType(transaction.type); // Ensure form type matches transaction
    setIsFormOpen(true);                
  };
  ...
  const handleAddExpenseClick = () => {
    ...
    // --- CHANGED HERE: Ensure we clear any previous editing data ---
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 transition-colors">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg border-4 border-green-500 flex flex-col justify-between">
            ...
            {/* --- CHANGED HERE: clear editingTransaction when adding income --- */}
            <Button onClick={() => { setEditingTransaction(null); setFormType('income'); setIsFormOpen(true); }} variant="secondary" className="mt-4 bg-green-50 text-green-700 hover:bg-green-100 dark:bg-green-900/30 dark:text-green-300">
              Add Income
            </Button>
          </motion.div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                {paginatedItems.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                    ...
                    <td className="px-6 py-4 flex justify-center space-x-2">
                      <button
                      onClick={() => handleEditClick(item)} // Use the dedicated handler
                      className="p-1.5 text-indigo-500 hover:bg-indigo-50 rounded-md transition-colors">
                        <Edit size={16} />
                      </button>
                      <button 
      ...
      {isFormOpen && (
        <TransactionForm 
          onClose={() => {
            setIsFormOpen(false);
            setEditingTransaction(null); // --- CHANGED HERE: Clear data when closing ---
          }} 
          type={formType} 
          transaction={editingTransaction} 
          existingBudgets={budgets} 
